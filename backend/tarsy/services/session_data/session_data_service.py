"""
Session Data Service - Main Facade.
...
from tarsy.services.session_data.chat_operations import ChatOperations
from tarsy.services.session_data.conversation_operations import ConversationOperations
from tarsy.services.session_data.interaction_operations import InteractionOperations
from tarsy.services.session_data.maintenance_operations import MaintenanceOperations
from tarsy.services.session_data.query_operations import QueryOperations
from tarsy.services.session_data.queue_operations import QueueOperations
from tarsy.services.session_data.session_operations import SessionOperations
from tarsy.services.session_data.stage_operations import StageOperations
from tarsy.services.session_data.tracking_operations import TrackingOperations


class SessionDataService:
    """
    Manages alert processing session data and audit trails.
    
    Provides centralized management of session lifecycle, LLM interaction tracking,
    MCP communication logging, and timeline reconstruction. Renamed from HistoryService
    to more accurately reflect its role in managing active session data, not just history.
    
    Architecture: This service acts as a facade that composes multiple specialized
    operation classes, each focused on a specific domain (sessions, stages, chats, etc.).
    All operations share a common infrastructure layer for database access and retry logic.
    """
    
    def __init__(self):
        """Initialize session data service with all specialized operations."""
        # Initialize base infrastructure
        self._infra = BaseSessionDataInfra()
        
        # Initialize all specialized services with shared infrastructure
    ...
    # Session lifecycle operations (delegate to SessionOperations)
    def create_session(
        self,
    ...
    # Stage execution operations (delegate to StageOperations)
    ...
    async def update_session_current_stage(
        ...
        return await self._stages.update_session_current_stage(
            session_id, current_stage_index, current_stage_id
        )
    ...
    # Interaction logging operations (delegate to InteractionOperations)
    ...
    # Query operations (delegate to QueryOperations)
    def get_sessions_list(
        ...
        return self._queries.get_sessions_list(
    ...
    # Maintenance operations (delegate to MaintenanceOperations)
    ...
    # Chat operations (delegate to ChatOperations)
    ...
    def get_session_conversation_history(
        ...
        return self._conversations.get_session_conversation_history(
            session_id, include_chat
        )
    ...
    # Tracking operations (delegate to TrackingOperations)
    ...
    # Queue management operations (delegate to QueueOperations)
